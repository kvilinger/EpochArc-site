...
4. Run `python3 scripts/build_events.py` and `python3 scripts/validate_forecasts.py` before deployment, then run `npm run build`. Event validation includes required fields, Claims, Editorial metadata, source references, enums, and score ranges; forecast validation includes signal-to-event linkage and `monitor_only` vs `consensus_gated` consistency.
